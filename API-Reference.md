...
| templateLast    | 'default'    | Contains a predefined template for the directive [ncyBreadcrumbLast](https://github.com/ncuillery/angular-breadcrumb/wiki/API-Reference#directive-ncybreadcrumblast) or HTML for a custom template. This property is ignored if `templateUrl` is defined.                                                                         |
| templateLastUrl | null         | Contains the path to a template file for the directive [ncyBreadcrumbLast](https://github.com/ncuillery/angular-breadcrumb/wiki/API-Reference#directive-ncybreadcrumblast). This property takes precedence over the `templateLast` property.                                                                                      |
...
<!-- Render "State A > State B > State C" -->
```
As the previous directive `ncyBreadcrumbLast`, this directive can have an optional attribute value: the template of the directive. Please note that the variable `ncyBreadcrumbLabel` is the full breadcrumb here (a bit :hankey: but PR is welcome):
```html
